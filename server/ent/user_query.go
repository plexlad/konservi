...
func (_q *UserQuery) QueryEntries() *EntryQuery {
...
func (_q *UserQuery) QueryMemberships() *ProjectMembershipQuery {
...
func (_q *UserQuery) WithEntries(opts ...func(*EntryQuery)) *UserQuery {
	query := (&EntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithMemberships(opts ...func(*ProjectMembershipQuery)) *UserQuery {
	query := (&ProjectMembershipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withEntries; query != nil {
		if err := _q.loadEntries(ctx, query, nodes,
			func(n *User) { n.Edges.Entries = []*Entry{} },
			func(n *User, e *Entry) { n.Edges.Entries = append(n.Edges.Entries, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *User) { n.Edges.Memberships = []*ProjectMembership{} },
			func(n *User, e *ProjectMembership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadEntries(ctx context.Context, query *EntryQuery, nodes []*User, init func(*User), assign func(*User, *Entry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(entry.FieldAuthorID)
	}
	query.Where(predicate.Entry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.EntriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadMemberships(ctx context.Context, query *ProjectMembershipQuery, nodes []*User, init func(*User), assign func(*User, *ProjectMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
